...
export class UserApiClient implements IUserApiClient {
	...
	async ConfirmEmail(command: ConfirmEmail.Command): Promise<boolean> {
		return await this.apiClient.post<ConfirmEmail.Command, boolean>(
	...
	async Register(command: Register.Command): Promise<void> {
		return await this.apiClient.postWithoutResponse<Register.Command>(
			`${this.url}/register`,
			command
		);
	}
	...
	async UpdateUserTheme(command: UpdateUserTheme.Command): Promise<void> {
		return await this.apiClient.put<UpdateUserTheme.Command>(
			`${this.url}/${command.id}/theme`,
			command
		);
	}

	async GetAllUsers(query: GetAllUsers.Query): Promise<PagedList<UserDto>> {
		return await this.apiClient.post<GetAllUsers.Query, PagedList<UserDto>>(
			`${this.url}/all`,
			query
		);
	}
