...
namespace YoutubeLinks.UnitTests.Features.Users.Commands
{
    public class RegisterFeatureTests
    {
        ...
        public async Task RegisterHandler_ThrowsValidationException_IfEmailExists()
        {
            ...
            var handler = new RegisterFeature.Handler(
                _passwordService,
                _userRepository,
                _emailService,
                _tokenService,
        ...
        [Fact]
        public async Task RegisterHandler_RegistersUserSuccessfully()
        {
            var command = new Register.Command
            {
                Email = "test@test.com",
            ...
            _tokenService.GenerateLink(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<LinkType>())
                         .Returns("link");
