using System.Security.Cryptography;
using System.Text;
using Microsoft.Extensions.Options;
using YoutubeLinks.Api.Abstractions;

namespace YoutubeLinks.Api.Auth;

public enum LinkType
{
    ConfirmEmail,
    ForgotPassword
}

public interface ITokenService
{
    string GenerateToken(string email);
    string GenerateLink(string email, string token, LinkType linkType);
}

public class TokenService : ITokenService
{
    private readonly string _baseUrl;
    private readonly IClock _clock;
    private readonly AuthOptions _options;

    public TokenService(
        IOptions<AuthOptions> options,
        IClock clock)
    {
        _options = options.Value;
        _clock = clock;
        _baseUrl = _options.FrontendUrl;
    }

    public string GenerateToken(string email)
    {
        var tokenData = $"{email}_{_clock.Current().Ticks}_{_options.SigningKey}";

        var hashedBytes = SHA256.HashData(Encoding.UTF8.GetBytes(tokenData));
        var token = Convert.ToBase64String(hashedBytes);

        return token;
    }

    public string GenerateLink(string email, string token, LinkType linkType)
    {
        var path = linkType switch
        {
            LinkType.ConfirmEmail => "confirmEmail",
            LinkType.ForgotPassword => "resetPassword",
            _ => throw new ArgumentOutOfRangeException(nameof(linkType),
                "Unsupported link type.") // TODO: custom exception
        };

        return $"{_baseUrl}/{path}?email={Uri.EscapeDataString(email)}&token={Uri.EscapeDataString(token)}";
    }
}